# Project Init AWS Internal Service

Module used to help create the ECS and Load balancing infrastructure for an internal Grpc or Http server. This is an
opinionated module, where the outcome should result in an internal service which registers the default healthcheck,
existing at
...
| <a name="input_container_port"></a> [container\_port](#input\_container\_port) | The port the internal service runs on. | `number` | `9001` | no |
...
| <a name="input_protocol_version"></a> [protocol\_version](#input\_protocol\_version) | The protocol version of the service. i.e. GRPC or HTTP | `string` | `"GRPC"` | no |
...
| <a name="input_sidecars"></a> [sidecars](#input\_sidecars) | Additional sidecar containers to run alongside the main container in the same task definition. | <pre>list(object({<br/>    name                  = string<br/>    image                 = string<br/>    essential             = optional(bool, false)<br/>    environment_variables = optional(list(object({ name = string, value = string })), [])<br/>    secrets               = optional(list(object({ name = string, valueFrom = string })), [])<br/>    port_mappings = optional(list(object({ containerPort = number, hostPort = number,<br/>    protocol = string })), [])<br/>    cpu    = optional(number, 0)<br/>    memory = optional(number, 0)<br/>  }))</pre> | `[]` | no |
| <a name="input_sidecars_cpu"></a> [sidecars\_cpu](#input\_sidecars\_cpu) | The total combined cpu value for all sidecars. | `number` | `0` | no |
| <a name="input_sidecars_memory"></a> [sidecars\_memory](#input\_sidecars\_memory) | The total combined memory value for all sidecars. | `number` | `0` | no |
...
| Name | Description |
|------|-------------|
| <a name="output_security_group_id"></a> [security\_group\_id](#output\_security\_group\_id) | The ID of the security group for the internal service |
| <a name="output_service_iam_role_arn"></a> [service\_iam\_role\_arn](#output\_service\_iam\_role\_arn) | The ARN of the IAM role for the internal service |
| <a name="output_service_iam_role_name"></a> [service\_iam\_role\_name](#output\_service\_iam\_role\_name) | The name of the IAM role for the internal service |
